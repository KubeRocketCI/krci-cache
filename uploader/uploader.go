...
func Uploader() error {
	...
	e.POST(uploadPath, upload)
	e.DELETE(uploadPath, uploaderDelete)
	e.DELETE(deletePath, deleteOldFilesOfDir)
	...
	if os.Getenv("UPLOADER_UPLOAD_CREDENTIALS") != "" {
		...
		c.Skipper = func(c echo.Context) bool {
			if (c.Request().Method == "HEAD" || c.Request().Method == "GET") && c.Path() != uploadPath && c.Path() != deletePath {
				return true
			}
			return false
		}
